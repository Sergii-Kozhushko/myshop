...
public class UtilServiceImpl implements UtilService {
     ...
     * @param add  True if the quantity should be added to the existing product quantity;
     *             false if it should be subtracted.
            ...
            if (item instanceof SupplyItem || item instanceof SaleItem) {
                ...
                if (item instanceof SupplyItem) {
                    p = productRepository.findById(((SupplyItem) item).getProduct().getId())
                            .orElseThrow(() ->
                                    new ProductNotFoundException(ErrorMassage.PRODUCT_NOT_FOUND_IN_INC_INVOICE));
                }
                if (item instanceof SaleItem) {
                    p = productRepository.findById(((SaleItem) item).getProduct().getId())
                            .orElseThrow(() ->
                                    new ProductNotFoundException(ErrorMassage.PRODUCT_NOT_FOUND_IN_DEC_INVOICE));
                }

                if (p != null) {
                    int quantity = (item instanceof SupplyItem) ? ((SupplyItem) item).getQuantity() :
                            ((SaleItem) item).getQuantity();
